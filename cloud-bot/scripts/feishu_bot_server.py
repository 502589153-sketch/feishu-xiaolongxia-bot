...
class FeishuHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            self._json(200, {"ok": True, "reply_sent": True})
        except urllib.error.HTTPError as e:
            print(f"[warn] reply send http error: {e.code}")
            self._json(200, {"ok": True, "reply_sent": False, "error": f"http_error:{e.code}"})
        except Exception as e:
            print(f"[warn] reply send error: {e}")
            self._json(200, {"ok": True, "reply_sent": False, "error": str(e)})
